...
function createMessage(status, deploymentUrl, deploymentId) {
  ...
  const message = {
    ...
    attachments: [{
      ...
      title: isProduction ? 'Production Deployment' : `${branch} Branch Deployment`,
      title_link: APP_URL,
...
function sendDiscordNotification(message) {
  return new Promise((resolve, reject) => {
    ...
    const discordMessage = {
      embeds: [{
        title: message.text,
        color: message.attachments[0].color === 'good' ? 0x00ff00 :
              message.attachments[0].color === 'danger' ? 0xff0000 : 0xffff00,
