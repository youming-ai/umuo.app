...
import { createMockFile, createMockTranscript, createMockSegment } from '@/__tests__/utils/test-helpers';
...
describe('数据库操作', () => {
  ...
  describe('文件操作', () => {
    ...
    it('应该按名称搜索文件', async () => {
      const mockFiles = [
        { id: 1, name: 'search-test.mp3', size: 1024, type: 'audio/mpeg' },
      ];
  ...
  describe('事务操作', () => {
    it('应该支持事务操作', async () => {
      ...
      await db.transaction('rw', db.files, db.transcripts, db.segments, async (tx) => {
        ...
        await tx.segments.bulkAdd([
          { ...mockSegments[0], transcriptId },
        ]);
      });
